...
func GetDB() *db.DB {
	myDB, err := db.OpenDB(gotiedotweb.DATABASE_DIR)
	if err != nil {
		panic(err)
	}
	return myDB
...
func (this *Album) GetGenreName() string {
	rawDoc := GetDoc(this.GenreId, gotiedotweb.GENRE_COL)
	return rawDoc.Value["name"].(string)

}

func GetGenreName(id uint64) string {
	rawDoc := GetDoc(id, gotiedotweb.GENRE_COL)
...
func GetBandsByGenre(id uint64) []DocWithID {
	database := GetDB()
	defer database.Close()
	collection := database.Use(gotiedotweb.BAND_COL)
	var query interface{}
...
func (this *DocWithID) GetLocation() string {
	...
	original := this.Value
	id := uint64(original["location_id"].(float64))
	rawDoc := GetDoc(id, gotiedotweb.LOCATION_COL)
...
func (this *DocWithID) AddAlbum(album Album) error {
	...
	band := Band{Name: original["name"].(string),
		LocationId: uint64(original["location_id"].(float64))}
	band.Albums = []Album{}
